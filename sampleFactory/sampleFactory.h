...
// Thu Mar 16 11:49:29 2017 by ROOT version 6.07/07
// from TTree tree/tree
// found on file: sampleInput/flatTuple_999.root
//////////////////////////////////////////////////////////
...
#ifdef sampleFactory_cxx
sampleFactory::sampleFactory(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("sampleInput/flatTuple_999.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("sampleInput/flatTuple_999.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("sampleInput/flatTuple_999.root:/ntuplizer");
...
Int_t sampleFactory::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t sampleFactory::LoadTree(Long64_t entry)
{
...
void sampleFactory::Init(TTree *tree)
{
...
void sampleFactory::Show(Long64_t entry)
{
...
Int_t sampleFactory::Cut(Long64_t entry)
{
